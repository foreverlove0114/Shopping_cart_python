...
def test_product_detail(logged_in_user):
    ...
    productId = get_product_id(logged_in_user)
    response = logged_in_user.get(f"{BASE_URL}/productDescription?productId={productId}")
...
def test_view_cart(logged_in_user):
    ...
    assert response.status_code == 200, f"Expected 200, got {response.status_code}"
    assert "proceed to checkout" in response.text.lower()
...
def test_remove_from_cart(user_with_cart_item):
    ...
    productId = get_product_id(user_with_cart_item)
    response = user_with_cart_item.get(f"{BASE_URL}/removeFromCart?productId={productId}")
...
def test_unauthorized_cart_access(session):
    ...
    assert response.status_code == 200, f"Expected 200, got {response.status_code}"
    assert "register here" in response.text.lower()
